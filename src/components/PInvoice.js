...
export default function PInvoice({dbpath,vsb}) {
  ...
  const fetchData = async() => { 
    ...
    if(result.data.phpresult[0]['invoice_no']<10)
    {
      document.getElementById("iid").innerHTML = "0"+result.data.phpresult[0]['invoice_no']; 
      // alert("hii")
    }
    else{
      document.getElementById("iid").innerHTML = result.data.phpresult[0]['invoice_no']; 
      // console.log("error");
...
// console.log("name", cart[0].name)
  return (
    <>
      ...
      <center>
          <h1 className="sp1" style={{fontWeight:'700', color:'rgb(67,35,130)'}}>Print Invoice</h1>
        </center>
        <br />
