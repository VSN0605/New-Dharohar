...
//     setImage(result.data.phpresult[0]['image']);
//     setAQuantity(result.data.phpresult[0]['quantity']);
//     addCart(result.data.phpresult[0]['qrid'], result.data.phpresult[0]['hsncode'], result.data.phpresult[0]['quantity'], result.data.phpresult[0]['name'], result.data.phpresult[0]['price'], result.data.phpresult[0]['price'], result.data.phpresult[0]['id'], result.data.phpresult[0]['description'], fprice, damount, gst);
//     calculateFprice(result.data.phpresult[0]['price'], damount, gst);
//   };
...
//             console.log(`Item ${index} - Available Quantity: ${cart[index].aqty}, Requested Quantity: ${cart[index].quantity}`);
...
//             // Convert fData2 to a JSON-like object for logging
//             let fData2Obj = {};
//             fData2.forEach((value, key) => {
//                 fData2Obj[key] = value;
//             });
//             console.log("fData2: ", fData2Obj);
...
//             // Append discount and GST for each item if applicable
//             fData.append('discount', cart[index].discount !== 0 || null ? cart[index].discount : 0);
//             fData.append('gst', cart[index].gst !== 0 || null ? cart[index].gst : 0);

//             console.log("Form Data for item:");
//             for (let pair of fData.entries()) {
//                 console.log(`${pair[0]}: ${pair[1]}`);
//             }

//             const strFdata = JSON.stringify(fData);
//             localStorage.setItem('fData', strFdata);
...
//       alert(`${gst}% GST and ${damount}% Discount Added !!`)
//     } else{
